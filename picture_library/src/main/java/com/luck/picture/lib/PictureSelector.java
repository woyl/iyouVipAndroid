...
public final class PictureSelector {
     ...
     * @param medias
     */
    public void externalPicturePreviewBack(int position, List<LocalMedia> medias, int enterAnimation,ResponListener<Boolean> responListener) {
        if (!DoubleUtils.isFastDoubleClick()) {
            PictureExternalPreviewActivity.setResponListener(responListener);
            Intent intent = new Intent(getActivity(), PictureExternalPreviewActivity.class);
            intent.putExtra(PictureConfig.EXTRA_PREVIEW_SELECT_LIST, (Serializable) medias);
            intent.putExtra(PictureConfig.EXTRA_POSITION, position);
            getActivity().startActivity(intent);
     ...
     * @param path
     */
    public void externalPictureVideoBack(String path,ResponListener<Boolean> responListener) {
        if (!DoubleUtils.isFastDoubleClick()) {
            PictureVideoPlayActivity.setResponListener(responListener);
            Intent intent = new Intent(getActivity(), PictureVideoPlayActivity.class);
            intent.putExtra("video_path", path);
            getActivity().startActivity(intent);
        }
