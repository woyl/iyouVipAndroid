...
import static com.jfkj.im.TIM.utils.NetWorkUtils.sIMSDKConnected;
...
public class TUIKitImpl {
    ...
    private static void initIM(Context context, int sdkAppID) {
        ...
        boolean init = TIMManager.getInstance().init(context, sdkConfig);
        OkLogger.e(init + "");
            ...
            public void onRefreshConversation(List<TIMConversation> conversations) {
               {
                    ...
                    for(TIMConversation timConversation:conversations){
                        if(timConversation.getType()!=C2C)
                        timConversationList.add(timConversation);
                    }
                    ConversationManagerKit.getInstance().onRefreshConversation(timConversationList);
                    for (IMEventListener l : sIMEventListeners) {
                        l.onRefreshConversation(timConversationList);
                    }
                }
                {
                    List<TIMConversation>  privatetimConversationList=new ArrayList<>();
                    for(TIMConversation timConversation:conversations){
                        if(timConversation.getType()==C2C)
                            privatetimConversationList.add(timConversation);
                    }
                    com.jfkj.im.TIM.zhf.ConversationManagerKit.getInstance().onRefreshConversation(privatetimConversationList);
                    for (IMEventListener l : sIMEventListeners) {
                        l.onRefreshConversation(privatetimConversationList);
                    }
                }
