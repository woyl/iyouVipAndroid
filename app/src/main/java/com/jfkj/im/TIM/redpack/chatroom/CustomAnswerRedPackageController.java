...
public class CustomAnswerRedPackageController {
    ...
    public static void onDraw(ICustomMessageViewGroup parent, final CustomMessage data, MessageInfo info) {
        ...
        if (data != null
                && !TextUtils.isEmpty(data.getRedId())
        ...
        tv_type.setText("广场红包");
        final String text = "不支持的自定义消息";
        if (data == null) {
            textView.setText(text);
        } else {
            if (!TextUtils.isEmpty(data.getRedDesc())) {
                textView.setText(data.getRedDesc());
            }else {
                textView.setText("祝大家财源广进");
            }
        }
    }
}
