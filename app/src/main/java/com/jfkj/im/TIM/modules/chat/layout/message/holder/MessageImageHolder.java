...
import com.jfkj.im.TIM.component.photoview.PhotoViewActivity;
import com.jfkj.im.TIM.component.picture.imageEngine.impl.GlideEngine;
import com.jfkj.im.TIM.component.video.VideoViewActivity;
    ...
    private void performImage(final MessageInfo msg, final int position) {
        ...
        contentImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                EventBus.getDefault().post(new ShowPicEvent(msg.getDataPath()));
...
//                TUIKit.getAppContext().startActivity(intent);
    ...
    private void performVideo(final MessageInfo msg, final int position) {
        ...
        if (!TextUtils.isEmpty(msg.getDataPath())) {
            String path = msg.getDataUri()== null ? msg.getDataPath(): msg.getDataUri().getPath();

            GlideEngine.loadCornerImage(contentImage, path, null, DEFAULT_RADIUS);
        } else {
    ...
    private void play(final MessageInfo msg) {
        if (msg.isSelf()) {
            TIMVideoElem timVideoElem = (TIMVideoElem) msg.getTIMMessage().getElement(0);
            statusImage.setVisibility(View.GONE);
            sendingProgress.setVisibility(View.GONE);
...
//            intent.putExtra(TUIKitConstants.CAMERA_VIDEO_PATH,Uri.fromFile(new File(timVideoElem.getVideoPath())));
//            intent.setFlags(FLAG_ACTIVITY_NEW_TASK);
//            App.getAppContext().startActivity(intent);
            Intent intent = new Intent(TUIKit.getAppContext(), PlayVideoActivity.class);
            intent.putExtra("video_url", Uri.fromFile(new File(timVideoElem.getVideoPath())).toString());
...
//            App.getAppContext().startActivity(intent);
            Intent intent = new Intent(TUIKit.getAppContext(), PlayVideoActivity.class);
            intent.putExtra("video_url", msg.getDataUri().toString());
            intent.putExtra("type","2");
            intent.addFlags(FLAG_ACTIVITY_NEW_TASK);
            App.getAppContext().startActivity(intent);
        }
