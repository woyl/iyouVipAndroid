...
import com.jfkj.im.TIM.modules.message.MessageRevokedManager;
        ...
        if (eleType == TIMElemType.SNSTips) {
            ...
            if (m.getRequestAddFriendUserList().size() > 0) {
                ...
                if (SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + Utils.ADDFRIEND) != null) {
                    if (SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + Utils.ADDFRIEND).length() > 0) {
                        String addfriend = SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + Utils.ADDFRIEND);
                        newfriendBeanList.addAll(JSON.parseArray(addfriend, NewfriendBean.class));
                    }
                }

                if (newfriendBeanList.size() > 0) {
                    for (int i = 0; i < newfriendBeanList.size(); i++) {
                        if (newfriendBeanList.get(i).getVisitorid().equals(m.getRequestAddFriendUserList().get(0))) {
                            if (newfriendBeanList.get(i).getState().equals("等待通过")) {
                                newfriendBeanList.get(i).setState("已接受");
                            }
                        }
                    }
                }
...
//                ToastUtil.toastLongMessage("已拒绝");
                List<NewfriendBean> newfriendBeanList = new ArrayList<>();
                if (SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + Utils.ADDFRIEND) != null) {
                    if (SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + Utils.ADDFRIEND).length() > 0) {
                        String addfriend = SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + Utils.ADDFRIEND);
                ...
                for (int i = 0; i < newfriendBeanList.size(); i++) {
                    if (newfriendBeanList.get(i).getVisitorid().equals(m.getDelFriendAddPendencyList().get(0))) {
                        if (newfriendBeanList.get(i).getState().equals("等待通过")) {
                            newfriendBeanList.get(i).setState("已拒绝");
                        }
                    }
                }
                SPUtils.getInstance(App.getAppContext()).put(Utils.APPID + Utils.ADDFRIEND, JSON.toJSONString(newfriendBeanList));
...
//            if(SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID + msg.getConversation().getLastMsg().getSender() + Utils.NODISTURB).equals("opennodisturb")){
//
//            }else {
//
//            }
    ...
    public void revokeMessage(boolean flag, final int position, final MessageInfo messageInfo) {
        if (!safetyCall()) {
            TUIKitLog.w(TAG, "unSafetyCall");
            return;
        }
    ...
    public void sendMessage(final MessageInfo message, boolean retry, final IUIKitCallBack callBack) {
        ...
        mCurrentConversation.sendMessage(message.getTIMMessage(), new TIMValueCallBack<TIMMessage>() {
            ...
            public void onSuccess(TIMMessage timMessage) {
                TUIKitLog.i(TAG, "sendMessage3 onSuccess");
                if (message.getElement() instanceof TIMCustomElem) {
                    TIMCustomElem elem = (TIMCustomElem) message.getElement();
        ...
        mCurrentConversation.getLocalMessage(unread > MSG_PAGE_COUNT ? unread : MSG_PAGE_COUNT
                , lastTIMMsg, new TIMValueCallBack<List<TIMMessage>>() {
                    ...
                    public void onError(int code, String desc) {
                        ...
                        callBack.onError(TAG, code, desc + "loginStatus:" + loginStatus);
                        TUIKitLog.e(TAG, "loadChatMessages() getMessage failed, code = " + code + ", desc = " + desc + "\t\t loginStatus : " + loginStatus );
                    }
