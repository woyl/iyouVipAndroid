...
public class ConversationManagerKit implements TIMRefreshListener, MessageRevokedManager.MessageRevokeHandler {
    ...
    public  static LinkedList<ConversationInfo> mTopLinkedList = new LinkedList<>();
    ...
    public void loadConversation(IUIKitCallBack callBack) {
        ...
        for (int i = 0; i < TIMConversations.size(); i++) {
            ...
            if (conversationInfo != null) {
                ...
                if(conversationInfo.isGroup()){
                    ...
                    if(conversationInfo.getId().equals(SPUtils.getInstance(App.getAppContext()).getString(Utils.AV_CHAT_ROOM_ID))||conversationInfo.getId().equals(Utils.CIRCLEROOMID)){
                    }else {
        ...
        //排序，imsdk加载处理的已按时间排序，但应用层有置顶会话操作，所有需根据置顶标识再次排序（置顶可考虑做到imsdk同步到服务器？）  @TGS#3TIG3IJGR  @TGS#3TIG3IJGR
    ...
    public void onRefreshConversation(List<TIMConversation> conversations) {
        ...
        for (int i = 0; i < conversations.size(); i++) {
            ...
            if(conversationInfo!=null&&conversationInfo.getId()!=null){
                if(conversationInfo.getId().equals(Utils.CIRCLEROOMID)){
                    return;
                }
                if(!SPUtils.getInstance(App.getAppContext()).getString(Utils.APPID +conversationInfo.getId()+Utils.NODISTURB,"closenodisturb").equals("closenodisturb")){
                    conversationInfo.setUnRead(0);
                }else {
                    conversationInfo.setUnRead(conversationInfo.getUnRead());
                }
            }
            if (conversation.getType() == TIMConversationType.System) {
                ...
                if (message.getElementCount() > 0) {
                    ...
                    if (eleType == TIMElemType.GroupSystem) {
                        ...
                        //    ToastUtil.toastLongMessage("您已经被邀请进群"+conversation.getGroupName());
                        }
                    }
                }
            }
            if (conversationInfo != null) {
                if(!conversationInfo.getId().equals(SPUtils.getInstance(App.getAppContext()).getString(Utils.AV_CHAT_ROOM_ID))){
                    infos.add(conversationInfo);
                }
        ...
        for (int j = 0; j < infos.size(); j++) {
            ...
            for (int i = 0; i < dataSource.size(); i++) {
                ...
                if (cacheInfo.getId().equals(update.getId())) {
                    ...
                    if(!update.getId().equals(SPUtils.getInstance(App.getAppContext()).getString(Utils.AV_CHAT_ROOM_ID))){
                        exists.add(update);
                        //infos.remove(j);
                        //需同步更新未读计数
     ...
     * @param groupSysEle
     */
    public void groupSystMsgHandle(TIMGroupSystemElem groupSysEle) {
    ...
    public void deleteConversation(String id, boolean isGroup) {
        ...
        if(mProvider.getDataSource()==null){
            return;
        }
        List<ConversationInfo> conversationInfos = mProvider.getDataSource();
        for (int i = 0; i < conversationInfos.size(); i++) {
