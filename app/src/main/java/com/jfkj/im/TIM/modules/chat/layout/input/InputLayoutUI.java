...
import com.jfkj.im.TIM.modules.chat.interfaces.IInputLayout;
...
abstract class InputLayoutUI extends LinearLayout implements IInputLayout {
    ...
    private boolean mSendGifDisable;
...
//        mShortcutArea = findViewById(R.id.shortcut_area);
        ...
        mTextInput.setInputType(TYPE_TEXT_FLAG_MULTI_LINE);
        LinearLayout ly_input = findViewById(R.id.ly_input);
        // 子类实现所有的事件处理
        dialoglevel = new Dialoglevel(mActivity,"VIP等级达到14级以后，才可解锁视频聊天功能。");
        init();
    ...
    protected void assembleActions() {
        ...
        if (!mSendPhotoDisable) {
            action.setIconResId(R.mipmap.chatting_btn_pic_gray);
        ...
        if (!mCaptureDisable) {
            ...
            action.setIconResId(R.mipmap.chatting_btn_photo_gray);
        ...
        if (!isGroup) {
            if (!mSendGifDisable) {
                ...
                action = new InputMoreActionUnit();
                action.setIconResId(R.mipmap.chatting_btn_video_gray);
                action.setTitleId(R.string.video_call);
                action.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (Integer.parseInt(UserInfoManger.getUserVipLevel()) >= 14) {
                            CustomAVCallUIController.getInstance().createVideoCallRequest(mActivity);
                        } else {
                            dialoglevel.show();
                ...
                action = new InputMoreActionUnit();
                action.setIconResId(R.mipmap.chatting_btn_sendgift_red);
                action.setTitleId(R.string.gift);
                action.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        startSendgift(name, uerid,true);
                    }
                });
                mInputMoreActionList.add(action);
            }
...
//        }
    ...
    protected abstract void startSendgift(String name, String userid,boolean flag);


    protected abstract void startreceiveggif(MessageInfo messageInfo);
    @Override
    ...
    @Override
    public void disableSendGifAction(boolean disable) {
        mSendGifDisable = disable;
    }
