...
public class CustomGameSubRedPackageController {
    ...
    public static void onDraw(ICustomMessageViewGroup parent, final CustomMessage data, MessageInfo info) {
        ...
        if (data.getSendRedPackageBean() != null && !TextUtils.isEmpty(data.getSendRedPackageBean().getBizTypeName())){
            tv_type.setText(data.getSendRedPackageBean().getBizTypeName());
        }
        final String text = "不支持的自定义消息";
        if (data == null) {
            textView.setText(text);
        } else {
            if (data.getPackageCustom()!= null && !TextUtils.isEmpty(data.getPackageCustom().getSendName())){
                textView.setText(data.getPackageCustom().getSendName());
            }
        }
    }
}
